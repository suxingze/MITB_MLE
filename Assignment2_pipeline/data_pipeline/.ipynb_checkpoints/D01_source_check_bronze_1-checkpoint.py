...
# --------------------------------
print("Checking Bronze Data Source 1...")
bronze_csv_path_1 = os.path.join(data_dir, "feature_clickstream.csv")
if not os.path.exists(bronze_csv_path_1):
    print(f"Bronze Data Source 1 '{bronze_csv_path_1}' does not exist. Please check the bronze data source.")
    raise SystemExit("Exiting the program due to missing label data source.")
print(f"Bronze Data Source 1 '{bronze_csv_path_1}' exists.")

# assuming the business t-1 timeliness
# check if current date's data is ready
bronze_pdf = pd.read_csv(bronze_csv_path_1)
bronze_pdf['snapshot_date'] = pd.to_datetime(bronze_pdf['snapshot_date'])

if bronze_pdf['snapshot_date'].max().date() < current_date:
    print(f"Current date's data is not ready. Last available date is {bronze_pdf['snapshot_date'].max().date()}. Expected at least {current_date}.")
    raise SystemExit("Exiting the program due to missing Current date's data.")
else:
    print(f"Current date's data is ready. Last available date is {bronze_pdf['snapshot_date'].max().date()}.")
