...
# --------------------------------
print("Checking Bronze Data Source 3...")
bronze_csv_path_3 = os.path.join(data_dir, "features_financials.csv")
if not os.path.exists(bronze_csv_path_3):
    print(f"Bronze Data Source 3 '{bronze_csv_path_3}' does not exist. Please check the bronze data source.")
    raise SystemExit("Exiting the program due to missing label data source.")
print(f"Bronze Data Source 3 '{bronze_csv_path_3}' exists.")

# assuming the business t-1 timeliness
# check if current date's data is ready
bronze_pdf = pd.read_csv(bronze_csv_path_3)
